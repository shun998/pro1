<%@page import="com.zs.core.Constants"%>
...
function selOrdersBut(formId,cbName){
...
function check(){
	if(!validateCheckbox("orderForm","orderIds","请至少选中一个订单"))
		return false;
	else
		...
		$.post(getContextPath()+"/admin/order/handleOrderStatus",{orderId:orderId,status:status},function(result){
			if(checkLogin(result)){
				if(result.handle=="success"){
					...
					if(status==9){
						os.html("交易关闭");
						tdTag.html("<a href=\"#\" onclick=\"if(confirm(\'确定要删除这个订单吗？\')) delOrder("+orderId+") \">删除订单</a>");
					}
					else if(status==3){
						os.html("退款成功");
						tdTag.html("<a href=\"#\" onclick=\"if(confirm(\'确定要删除这个订单吗？\')) delOrder("+orderId+") \">删除订单</a>");
...
function sendGoods(orderId,status,e){
	...
	$("#ok").click(function(){
		...
		$.post(getContextPath()+"/admin/order/handleOrderStatus",{orderId:orderId,status:status,orderPostname:orderPostname,orderPostcode:orderPostcode},function(result){
			if(checkLogin(result)){
				if(result.handle=="success"){
					if(status==4){
						os.html("已发货");
...
function delOrder(orderId){
	location.href="${pageContext.request.contextPath}/admin/order/delOrder?orderId="+orderId;
}
</script>
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
         <div class="row">
          ...
          <div class="table-responsive ">
          	<form id="orderForm" method="post" action="${pageContext.request.contextPath}/admin/order/delOrderByIds" onsubmit="return check()">
          	<div class="row">
