...
<script type="text/javascript">
		...
		function(){
			...
			if(cates && cates.length>0){
				...
				for(var i=0;i<cates.length;i++){
					//上方导航下拉菜单-商品分类
					var s="<li><a href='"+getContextPath()+"/goods/getGoodsByCate?cateId="+cates[i].cateId+"'>"+(cates[i].cateName).escapeHTML()+"</a></li>";
					$("#dropdown_cate").append(s);
					//左侧导航listgroup-商品分类						
					if(cates[i].cateId==cateId)
						s="<a href='"+getContextPath()+"/goods/getGoodsByCate?cateId="+cates[i].cateId+"' class='list-group-item active'>"+cates[i].cateName.escapeHTML()+"</a>";
					else
						s="<a href='"+getContextPath()+"/goods/getGoodsByCate?cateId="+cates[i].cateId+"' class='list-group-item '>"+cates[i].cateName.escapeHTML()+"</a>";
					$("#left_cate").append(s);
				}
			}
			else{
			...
			function(result){
				...
				if(result.login=="yes"){
					...
					if(result.redirUrl){
						...
						var len=$('#info').children('li').length;
						if(len==5)//导航条已变为登录后状态
							location.href=getContextPath()+"/index";
						else{
							$("#info").html("<li id='li1'><span>"+userName.escapeHTML()+"您好，欢迎来到小白专卖店！</span>"+										
	...
	function reg(){
		if(!validateRequired("regForm","userName","用户名不能为空")||
				!validateRequired("regForm","userPass","密码不能为空")||
				!validatePass("regForm","userPass","userPass1","两次密码输入不一致")||
				!validateEmail("regForm","userEmail","请输入有效电子邮件地址")){
			return;
		}
		var userName=$("#regForm #userName").val();
		var userPass=$("#regForm #userPass").val();
		var userPass1=$("#regForm #userPass1").val();
	...
	function showCart(){
		...
		if(goodsList!=null&&goodsList.length>0){
			...
			for(var i=0;i<goodsList.length;i++){
				...
				s+="<input type='hidden' name='size' value='"+goods.size.escapeHTML()+"'/>";
				s+="<input type='hidden' name='color' value='"+goods.color.escapeHTML()+"'/>";
				//s+="<input type='hidden' name='goodsName' value='"+goods.name.escapeHTML()+"'/>";
				//s+="<input type='hidden' name='pic' value='"+goods.pic.escapeHTML()+"'/>";
				...
				s+="<tr><td>"+(i+1)+"</td>";
				s+="<td><img src='"+getContextPath()+goods.pic.escapeHTML()+"' width='50' height='50'></td>";
				s+="<td><a href='"+getContextPath()+"/goods/getGoodsDetailById?goodsId="+goods.id+"&cateId="+goods.cateId+"'>"+goods.name.escapeHTML()+"</a><br>"+goods.size.escapeHTML()+"&nbsp;&nbsp;"+goods.color.escapeHTML()+"</td>";
	...
	function changeCart(index){
		var newnum=$("input[name='num']").eq(index).val();
		...
		<div class="col-md-12 text-right">
			...
			<ol class="breadcrumb" id="info">
				<c:if test="${sessionScope['_LOGIN_USER_']==null}">
					<li id="li1"><span>游客您好，欢迎来到小白专卖店！</span>
					<a href="#loginFormModal" data-toggle="modal">[登录]</a>&nbsp;<a href="#regFormModal" data-toggle="modal">[新用户注册]</a></li>
					<li><a href="#" onclick="showCart()">购物车 <span class="badge" id="cartBadge"></span></a></li>
				</c:if>
				<c:if test="${sessionScope['_LOGIN_USER_']!=null}">
					<li id="li1"><span>${fn:escapeXml(sessionScope['_LOGIN_USER_'].userName) }您好，欢迎来到小白专卖店！</span>										
					<li><a href="#" onclick="showCart()">购物车 <span class="badge" id="cartBadge"></span></a></li>
					<li><a href="${pageContext.request.contextPath}/order/getMyOrders">我的订单</a></li>
					<li><a href="${pageContext.request.contextPath}/usercenter/index">个人中心</a></li>
					<li><a href="#" onclick="logout()">退出登录</a></li>
		...
		<div class="col-md-12">
			<nav class="navbar navbar-default navbar-static-top" role="navigation">
				...
				<div class="navbar-header">
					...
					</button> <a class="navbar-brand" href="#">小白专卖店</a>
				</div>
						...
						<li class="dropdown" >
							...
							<ul class="dropdown-menu">
								<c:if test="${param.keyword!=null}">
									<li>
										<a href="${pageContext.request.contextPath}/goods/searchGoods?keyword=${fn:escapeXml(param.keyword)}&sort=1">按价格从低到高</a>
									</li>
									<li>
										<a href="${pageContext.request.contextPath}/goods/searchGoods?keyword=${fn:escapeXml(param.keyword)}&sort=2">按价格从高到低</a>
									</li>								
									<li class="divider">
									</li>
									<li>
										<a href="${pageContext.request.contextPath}/goods/searchGoods?keyword=${fn:escapeXml(param.keyword)}&sort=3">按销量从高到低</a>
									</li>
									<li>
										<a href="${pageContext.request.contextPath}/goods/searchGoods?keyword=${fn:escapeXml(param.keyword)}&sort=4">按销量从低到高</a>
									</li>
								</c:if>
								<c:if test="${param.keyword==null}">
