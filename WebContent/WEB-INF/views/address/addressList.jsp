		...
		$.post(getContextPath()+"/address/getAddressById",{addrId:addrId},function(result){
			if(checkLogin(result)){
				if(result.addr){
					...
					$("#addrProvince").val(result.addr.addrProvince.escapeHTML());
		...
		<div class="col-md-10">
			...
			<tbody>
				...
				<tr>
					 <td>${vs.count }</td>
					 <td>
					 	${fn:escapeXml(a.addrProvince)} ${fn:escapeXml(a.addrCity)} ${fn:escapeXml(a.addrArea)}
					 </td>
					 <td>
					 	${fn:escapeXml(a.addrContent)}</td><td>${fn:escapeXml(a.addrReceiver)}
					 </td>
					 <td> 
